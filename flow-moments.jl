...
@add_arg_table! s begin
    "--src", "-s"
        help = "source dir"
        arg_type = String
        default = "s3://lizz.ssd/datasets/moments_merge"
    "--dst", "-t"
        help = "target dir"
        arg_type = String
        default = "s3://lizz.ssd/datasets/moments_merge_flow"
    "--list", "-l"
        help = "list file"
        arg_type = String
        default = "moments_merge_list.txt"
    "--split"
...
errors = @showprogress "batches " pmap(batches) do batch
    ...
    try
        ...
        writedlm(joinpath(batchdir, "list.txt"), (*).(Ref("videos/"), basename.(batch)))

        # download
        writedlm(joinpath(batchdir, "commands.txt"), (*).(Ref("cp $(src)/"), batch, Ref(" videos/"), basename.(batch)))
        Shell.run("""
        cd '$(batchdir)'
        ...
        # encode & upload
        names = getindex.(splitext.(basename.(batch)), 1)
        classes = basename.(dirname.(batch))
        for (v, c) in zip(names, classes)
            Shell.run("""
            cd '$(batchdir)'
            ffmpeg -hide_banner -loglevel panic -i 'png/$(v)/flow_%05d.png' -c:v libx265 -crf 10 -x265-params log-level=error 'out/$(v).mp4' -y
            s3cmd put 'out/$(v).mp4' '$(joinpath(dst, c, "$(v)_flow.mp4"))' -q
            """)
...
writedlm("moments_merge-flow-split$(split)_of_$(splits)-errors.log", errors)

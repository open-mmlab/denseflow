...
function uncast(img)
    ...
    println("parsed bound x $bx, bound y $by")
    epsx = base * bx
    epsy = base * by
    flowx = (img[:, :, 1] .- 128) .* epsx
    flowy = (img[:, :, 2] .- 128) .* epsy
    return flowx', flowy'
end

for id = 0:827
    println(id)
    flowx = data["flow_x_$(lpad(id, 5, "0")).jpg"]
    flowy = data["flow_y_$(lpad(id, 5, "0")).jpg"]
    img = cast(flowx, flowy)
    cv2.imwrite("out/flow_$(lpad(id, 5, "0")).png", img, (cv2.IMWRITE_PNG_COMPRESSION, 9))
    # rm("a.h5")
    # h5write("a.h5", "img", img)
    a = cv2.imread("out/flow_$(lpad(id, 5, "0")).png")
    # b = round(mean(a[:, :, 3]) / 4) * 4
    # @assert b == img[1, 1, 3]
    xx, yy = uncast(a)
    # println(size(xx), size(flowx))
    println("x diff ", maximum(abs.(flowx .- xx)), ", y diff", maximum(abs.(flowy .- yy)))
end

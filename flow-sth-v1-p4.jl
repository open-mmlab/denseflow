...
@add_arg_table! s begin
    ...
    "--dst", "-t"
        help = "target dir"
        arg_type = String
        default = "s3://lizz.ssd/datasets/something-something-v1_flow_p4"
    "--list", "-l"
        help = "list file"
        arg_type = String
...
errors = @showprogress "batches " pmap(batches) do batch
    ...
    try
        ...
        Shell.run("""
        cd '$(batchdir)'
        CUDA_VISIBLE_DEVICES=$(myid() % 8) denseflow -s=4 -st=png -o=png list.txt
        """)
        # release gpu
        ...
        for v in names
            Shell.run("""
            cd '$(batchdir)'
            ffmpeg -hide_banner -loglevel panic -i 'png/$(v)/flow_p4_%05d.png' -c:v libx265 -crf 10 -x265-params log-level=error 'out/$(v).mp4' -y
            s3cmd put 'out/$(v).mp4' '$(joinpath(dst, "$(v)_flow_p4.mp4"))' -q
            """)
...
writedlm("something-something-v1-flow-p4-split$(split)_of_$(splits)-errors.log", errors)

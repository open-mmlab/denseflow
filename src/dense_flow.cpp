...
void calcDenseFlow(std::string file_name, int bound, int type, int step, std::vector<std::vector<uchar>> &output_x,
                   std::vector<std::vector<uchar>> &output_y, std::vector<std::vector<uchar>> &output_img) {

    VideoCapture video_stream(file_name);
    CHECK(video_stream.isOpened()) << "Cannot open video stream \"" << file_name << "\" for optical flow extraction.";
    ...
    for (int iter = 0;; iter++) {
        ...
        if (!initialized) {
            ...
            switch (type) {
            case 0: {
                calcOpticalFlowFarneback(prev_gray, capture_gray, flow, 0.702, 5, 10, 2, 7, 1.5,

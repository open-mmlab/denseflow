...
@add_arg_table! s begin
    ...
    "--dst", "-t"
        help = "target dir"
        arg_type = String
        default = "s3://lizz.ssd/datasets/something-something-v1_flow_m4"
    "--list", "-l"
        help = "list file"
        arg_type = String
...
errors = @showprogress "batches " pmap(batches) do batch
    ...
    try
        ...
        for v in names
            Shell.run("""
            cd '$(batchdir)'
            ffmpeg -hide_banner -loglevel panic -i 'png/$(v)/flow_m4_%05d.png' -c:v libx265 -crf 10 -x265-params log-level=error 'out/$(v).mp4' -y
            s3cmd put 'out/$(v).mp4' '$(joinpath(dst, "$(v)_flow_m4.mp4"))' -q
            """)
...
writedlm("something-something-v1-flow-m4-split$(split)_of_$(splits)-errors.log", errors)
